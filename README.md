...
После добавления карточки через UI доступны `entity`, название, расположение, MDI-иконка, режимы включения/выключения и entity поддержания тепла.

Режимы чая намеренно не отображаются в карточке. Старые `modes` в YAML можно оставить — они игнорируются.
...
Ползунок температуры изменяется с шагом 5°C, обновляет значение во время перетаскивания и отправляет `set_temperature` после отпускания.
